...
public class EstudianteServiceImpl implements IEstudianteService { // Define la clase EstudianteServiceImpl que implementa la interfaz IEstudianteService
    ...
    @Override
    @Cacheable(value = "estudiante", key = "#id") // Cachea en Redis con clave "estudiante::[id]"
    @Transactional(readOnly = true)
    public EstudianteDTO obtenerEstudiantePorId(Long id) {
        // Validación del ID
        if (id == null || id <= 0) {
            throw new IllegalArgumentException("ID de estudiante no válido: " + id);
        }

        // Buscar en la BD (con manejo de excepciones)
    ...
    @Override
    @Transactional
    @CacheEvict(value = {"estudiante", "estudiantes", "estudiantesActivos"}, key = "#id")
    public void eliminarFisicamente(Long id) {
        Estudiante estudiante = estudianteRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Estudiante no encontrado con ID: " + id));

        // Validar que el estudiante no tenga materias asociadas
        if (!estudiante.getMaterias().isEmpty()) {
            throw new IllegalStateException("No se puede eliminar el estudiante porque tiene materias asignadas");
        }
